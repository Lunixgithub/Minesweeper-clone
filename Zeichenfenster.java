 ...
 * auf der Objekte der Klassen Rechteck, Kreis und Dreieck sowie Turtle
 * dargestellt
 * werden können.
...
class Zeichenfenster {
  ...
  abstract class GrafikSymbol {
    ...
    /**
     * Der Konstruktor erzeugt ein rotes Symbol in der linken oberen Ecke des
     * Fensters.
     */
    GrafikSymbol() {
      x = 10;
  ...
  class TextIntern extends GrafikSymbol {
    ...
    void Zeichnen(Graphics g) {
      ...
      if (winkel == 0) {
        ...
        // g2.rotate(DrehwinkelGeben (winkel), x + text.length() * size / 4, y + size /
        // 2);
  ...
  private class FigurenElementRechteck extends FigurenElement {
     ...
     * 
     * @param x      x-Koordinate der linken oberen Ecke des Rechtecks relativ zum
     *               Aufhängepunkt.
     * @param y      y-Koordinate der linken oberen Ecke des Rechtecks relativ zum
     *               Aufhängepunkt.
    ...
    void ElementZeichnen(Graphics2D g, double größe, int x, int y) {
      g.setColor(c);
      g.fill(new Rectangle2D.Double(x + größe * xe / 100.0, y + größe * ye / 100.0, größe * breite / 100.0,
          größe * höhe / 100.0));
    }
    ...
    void ElementZuForm(Path2D.Double p, double größe, int x, int y) {
      p.append(new Rectangle2D.Double(x + größe * xe / 100.0, y + größe * ye / 100.0, größe * breite / 100.0,
  ...
  private class FigurenElementEllipse extends FigurenElement {
    ...
    void ElementZeichnen(Graphics2D g, double größe, int x, int y) {
      g.setColor(c);
      g.fill(new Ellipse2D.Double(x + größe * xe / 100.0, y + größe * ye / 100.0, größe * breite / 100.0,
          größe * höhe / 100.0));
    }
    ...
    @Override
    void ElementZuForm(Path2D.Double p, double größe, int x, int y) {
      p.append(new Ellipse2D.Double(x + größe * xe / 100.0, y + größe * ye / 100.0, größe * breite / 100.0,
          größe * höhe / 100.0), false);
    }
  }
  ...
  class TurtleIntern extends GrafikSymbol {
    ...
    private class HintergrundBild {
       ...
       */
      HintergrundBild() {
        bild = new BufferedImage(Zeichenfenster.MalflächenBreiteGeben(), Zeichenfenster.MalflächenBreiteGeben(),
    ...
    boolean Berührt() {
      for (GrafikSymbol g : zeichenfläche.alleSymbole) {
        if ((g != this) && g.IstInnerhalb(x, y) && g.sichtbar
            && (!(g instanceof TurtleIntern) || ((TurtleIntern) g).symbolSichtbar)) {
          return true;
        }
      }
      return false;
     ...
     * @return true, wenn die Turtlekoordinaten innerhalb einer Grafikfigur in der
     *         angegebenen Farbe sind
     */
      ...
      for (GrafikSymbol g : zeichenfläche.alleSymbole) {
        if ((g != this) && g.IstInnerhalb(x, y) && g.sichtbar) {
          if (g instanceof TurtleIntern) {
            ...
            LinkedList<FigurenElement> figur = ((t.eigeneFigur == null) || (t.eigeneFigur.size() == 0))
                ? t.standardFigur
                : t.eigeneFigur;
  ...
  class FigurIntern extends GrafikSymbol {
    ...
    void FormErzeugen() {
      ...
      double größe = h > b ? b : h;
      if (standardFigur != null) {
        LinkedList<FigurenElement> figur = ((eigeneFigur == null) || (eigeneFigur.size() == 0)) ? standardFigur
            : eigeneFigur;
        synchronized (figur) {
          for (FigurenElement e : figur) {
            Path2D.Double p = new Path2D.Double();
    ...
    /**
     * Testet, ob die Figur eine (sichtbare) Grafik-Figur in der angegebenen Farbe
     * berührt.
     * Bei Überlappungen
     * 
     * @param farbe die Farbe, die die berührte Figur haben muss.
     * @return true, wenn die Figurkoordinaten innerhalb einer Grafikfigur in der
     *         angegebenen Farbe sind
     */
    boolean Berührt(String farbe) {
      Color c2 = FarbeCodieren(farbe);
     ...
     * Alle Werte beziehen sich auf eine Figur der Größe 100 und den Koordinaten
     * (0|0) in der Mitte des Quadrats
     * 
